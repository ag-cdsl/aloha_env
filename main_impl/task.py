...
ALOHA_ASSET_PATH = (
    Path.home()
    / ".local/share/ov/pkg/isaac_sim-2022.1.1/standalone_examples/aloha_env/aloha_rl/ALOHA_with_sensor_02.usd"
).as_posix()

SCENE_ASSET_PATH = (
    Path.home()
    / ".local/share/ov/pkg/isaac_sim-2022.1.1/standalone_examples/aloha_env/aloha_rl/airi_room_03/airi_room_03.usd"
).as_posix()
...
class AlohaTask(BaseTask):
    def __init__(
        ...
        self.wheels_controller = DifferentialController(
            name="simple_control", wheel_radius=0.068, wheel_base=0.34
        )
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        table_height = 0.7
        cube_size = 0.05
        self.cube_default_translation = np.array(
            [1.5, -0.2, table_height + cube_size / 2]
        )
        self.render_products = []
        camera_paths = [f"/World/scene_{i}/aloha/fl_link4/visuals/realsense/husky_rear_left" for i in range(self.num_envs)]
        for scene_id in range(self.num_envs):
            scene_prim_path = f"/World/scene_{scene_id}"
            create_prim(prim_path=scene_prim_path, position=(0, 0 + 5 * scene_id, 0))

            create_prim(
                prim_path=f"{scene_prim_path}/kitchen",
                translation=(0, 0, 0),
                usd_path=SCENE_ASSET_PATH,
            )
            ...
            render_product = self.rep.create.render_product(camera_paths[scene_id], resolution=(500, 500))
        ...
        output_directory = "/mnt/hdd5/Datasets/isaac_writer"
        ...
        self.robots = ArticulationView(
            prim_paths_expr=f"/World/scene_*/aloha", name="aloha_view"
        )
        self.cubes = ArticulationView(
            prim_paths_expr=f"/World/scene_*/cube", name="cube_view"
        )
        self.tables = ArticulationView(
            prim_paths_expr=f"/World/scene_*/table", name="table_view"
        )
        self.tlocs = ArticulationView(
            prim_paths_expr=f"/World/scene_*/target_location", name="tloc_view"
        )
    ...
    def post_reset(self) -> None:
        self._wheel_dof_indices = [
            self.robots.get_dof_index(self._wheel_dof_names[i])
            for i in range(self._num_wheel_dof)
        ]
        self._gripper1_dof_indices = [
            self.robots.get_dof_index(self._gripper1_dof_names[i])
            for i in range(self._num_gripper1_dof)
        ]
        self._gripper2_dof_indices = [
            self.robots.get_dof_index(self._gripper2_dof_names[i])
            for i in range(self._num_gripper2_dof)
        ]
        self._arm1_dof_indices = [
            self.robots.get_dof_index(self._arm1_dof_names[i])
            for i in range(self._num_arm1_dof)
        ]
        self._arm2_dof_indices = [
            self.robots.get_dof_index(self._arm2_dof_names[i])
            for i in range(self._num_arm2_dof)
        ]
    ...
    def get_observations(self) -> dict:
        ...
        grip_1_jpos = self.robots.get_joint_positions(
            joint_indices=self._gripper1_dof_indices
        )
        grip_2_jpos = self.robots.get_joint_positions(
            joint_indices=self._gripper2_dof_indices
        )
        arm_1_jpos = self.robots.get_joint_positions(
            joint_indices=self._arm1_dof_indices
        )
        arm_1_jvel = self.robots.get_joint_velocities(
            joint_indices=self._arm1_dof_indices
        )
        arm_2_jpos = self.robots.get_joint_positions(
            joint_indices=self._arm2_dof_indices
        )
        arm_2_jvel = self.robots.get_joint_velocities(
            joint_indices=self._arm2_dof_indices
        )
    ...
    def calculate_metrics(self) -> dict:
        tloc_pos = self.obs["observation"][:, 48:51]
        cube_pos = self.obs["observation"][:, 41:44]
    ...
    def pre_physics_step(self, actions):
        ...
        self.robots.set_joint_velocities(
            wheel_vels, joint_indices=self._wheel_dof_indices
        )
        ...
        self.robots.set_joint_position_targets(
            jpos, joint_indices=self._gripper1_dof_indices
        )
        ...
        self.robots.set_joint_position_targets(
            jvels, joint_indices=self._arm1_dof_indices
        )
        ...
        jpos = torch.stack([cmd, cmd], axis=-1)
        self.robots.set_joint_position_targets(
            jpos, joint_indices=self._gripper2_dof_indices
        )
        ...
        self.robots.set_joint_position_targets(
            jvels, joint_indices=self._arm2_dof_indices
        )
